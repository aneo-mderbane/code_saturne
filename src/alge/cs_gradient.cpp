...

{
  ...
  ctx.parallel_for_i_faces(m, [=] CS_F_HOST_DEVICE (cs_lnum_t f_id) {
    ...
    cs_dispatch_sum<cs_real_t>(&cocg[ii][0], dc[0]*dc[0]*ddc, CS_DISPATCH_SUM_ATOMIC);
    cs_dispatch_sum<cs_real_t>(&cocg[ii][1], dc[1]*dc[1]*ddc, CS_DISPATCH_SUM_ATOMIC);
    cs_dispatch_sum<cs_real_t>(&cocg[ii][2], dc[2]*dc[2]*ddc, CS_DISPATCH_SUM_ATOMIC);
    cs_dispatch_sum<cs_real_t>(&cocg[ii][3], dc[0]*dc[1]*ddc, CS_DISPATCH_SUM_ATOMIC);
    cs_dispatch_sum<cs_real_t>(&cocg[ii][4], dc[1]*dc[2]*ddc, CS_DISPATCH_SUM_ATOMIC);
    cs_dispatch_sum<cs_real_t>(&cocg[ii][5], dc[0]*dc[2]*ddc, CS_DISPATCH_SUM_ATOMIC);

    // Update cocg for cell jj
    cs_dispatch_sum<cs_real_t>(&cocg[jj][0], dc[0]*dc[0]*ddc, CS_DISPATCH_SUM_ATOMIC);
    cs_dispatch_sum<cs_real_t>(&cocg[jj][1], dc[1]*dc[1]*ddc, CS_DISPATCH_SUM_ATOMIC);
    cs_dispatch_sum<cs_real_t>(&cocg[jj][2], dc[2]*dc[2]*ddc, CS_DISPATCH_SUM_ATOMIC);
    cs_dispatch_sum<cs_real_t>(&cocg[jj][3], dc[0]*dc[1]*ddc, CS_DISPATCH_SUM_ATOMIC);
    cs_dispatch_sum<cs_real_t>(&cocg[jj][4], dc[1]*dc[2]*ddc, CS_DISPATCH_SUM_ATOMIC);
    cs_dispatch_sum<cs_real_t>(&cocg[jj][5], dc[0]*dc[2]*ddc, CS_DISPATCH_SUM_ATOMIC);
                       ...
                       cs_gradient_quantities_t      *gq)
{
  ...
  for (cs_lnum_t c_id = 0; c_id < ((n_cells_ext < 5) ? n_cells_ext : 5); c_id++) {
    printf("cocg[%ld]:", c_id);
    for (cs_lnum_t ll = 0; ll < 6; ll++) {
...
static CS_F_HOST_DEVICE inline void
_math_6_inv_cramer_sym_in_place_local(cs_cocg_t  a[6])
{
  cs_real_t a00 = a[1]*a[2] - a[4]*a[4];
  cs_real_t a01 = a[4]*a[5] - a[3]*a[2];
  cs_real_t a02 = a[3]*a[4] - a[1]*a[5];
                           ...
                           cs_cocg_t                     (*restrict cocg)[6])
{
  ...
  cs_lnum_t *cell_b_faces_idx
    = (cs_lnum_t *) ma->cell_b_faces_idx;
  cs_lnum_t *cell_b_faces
    = ma->cell_b_faces;

  cs_real_3_t * b_face_u_normal
    = (cs_real_3_t *)fvq->b_face_u_normal;
  cs_real_t *b_dist
    = (cs_real_t *)fvq->b_dist;
  cs_real_3_t *diipb
    = (cs_real_3_t *)fvq->diipb;
  cs_lnum_t  *b_cells 
    = m->b_cells;
                     ...
                     cs_real_3_t           *restrict grad)
{
  ...
  if (recompute_cocg)
    _recompute_lsq_scalar_cocg_orig(m,
                               fvq,
                               bc_coeffs,
                               cocgb,
                               cocg);
                                  ...
                                  cs_real_3_t           *restrict grad)
{
  ...
  if (is_porous == 0) {
    ...
    _f_ext_d[0] = 0.; _f_ext_d[1] = 0.; _f_ext_d[2] = 0.;
  ...
  case CS_GRADIENT_GREEN_LSQ:
    {
      ...
      if (w_stride == 6 && c_weight != nullptr)
        ...
        if (cs_get_device_id() > -1)
          _lsq_scalar_gradient_hyd_p_gather<CS_E2N_SUM_GATHER>
              (mesh,
              fvq,
              halo_type,
              recompute_cocg,
